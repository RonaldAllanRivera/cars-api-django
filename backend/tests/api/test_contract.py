...
tested against (mobile/src/api/__fixtures__). They were generated by the
Laravel API; this builds the same data, calls the real endpoints and asserts
every response still matches - values, keys and key order.
...
def test_the_live_api_matches_every_committed_fixture(responses):
    ...
    for name, payload in responses.items():
        ...
        # Same bytes as Laravel's JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE:
        # this also pins key order.
